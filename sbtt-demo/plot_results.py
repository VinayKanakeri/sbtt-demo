...
drop_ratio = np.load('random_lorenz_eval_random_result_drop_ratio.npy')

# Lorenz plots
lorenz_random = np.load('random_lorenz_eval_random_result_mse.npy')
lorenz_uniform = np.load('uniform_lorenz_eval_uniform_result_mse.npy')
...
ax.legend()
plt.savefig('./results_1/uniform_random_lorenz.png')

# calcium2p plots
calcium2p_random = np.load('random_calcium2p_eval_random_result_mse.npy')
calcium2p_uniform = np.load('uniform_calcium2p_eval_uniform_result_mse.npy')
fig, ax = plt.subplots()
ax.plot(drop_ratio, calcium2p_random, linewidth=2, marker='o', label='calcium2p - random')
ax.plot(drop_ratio, calcium2p_uniform, linewidth=2, marker='o', label='calcium2p - uniform')
ax.set_xlabel('Fraction dropped samples')
ax.set_ylabel('MSE')
ax.grid(True)
ax.set_title('Uniform and random sampling comparison for calcium2p data')
ax.legend()
plt.savefig('./results_1/uniform_random_calcium2p.png')
